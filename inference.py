...
checkpoint_path = '/home/taimur/Documents/Online Courses/Fourth Brain/Projects/Audio_super_res/ml_deployment/saved_model'
default_inf_device = 'cpu',

#choices=['cpu'] + [f'cuda:{d}' for d in range(torch.cuda.device_count())], type=str,

# Load checkpoint
checkpoint = torch.load(checkpoint_path, map_location=default_inf_device)

# Initializing model, optimizer, criterion and scaler
model = Generator(wavenet=WaveNet())
model.to(default_inf_device)
